...
PHI_FIELDS: tuple[PhiField, ...] = (
    # ── PatientCensus ──────────────────────────────────────────────────────
    PhiField("PatientCensus", "fhir_patient_id", "unique_identifier",
             "Direct patient identifier from the EHR system"),
    PhiField("PatientCensus", "age_years",       "age",
             "HIPAA identifier when combined with geography or condition"),
    PhiField("PatientCensus", "gender",          "demographic",
             "Low sensitivity individually; PHI in combination"),
    PhiField("PatientCensus", "country_code",    "geographic_subdivision",
             "Country-level geography; phi when combined with age/condition"),
    PhiField("PatientCensus", "extra",           "other_identifier",
             "Raw FHIR resource may contain names, dates, addresses"),

    # ── SupplyTransaction ──────────────────────────────────────────────────
    PhiField("SupplyTransaction", "facility_id",   "geographic_subdivision",
             "Facility identifier can be used to infer patient population"),
    PhiField("SupplyTransaction", "lot_number",     "device_identifier",
             "Lot numbers can link back to individual patient records"),
    PhiField("SupplyTransaction", "fhir_resource_id", "unique_identifier",
