"""mSupply sync configuration and audit log models."""
...
class MSupplySyncStatus(str, enum.Enum):
    pending = "pending"
    running = "running"
    completed = "completed"
    failed = "failed"
...
class MSupplySyncConfig(Base):
    """Stores mSupply instance connection details and mapping config."""

    __tablename__ = "msupply_sync_config"
...
class MSupplySyncLog(Base):
    """Audit trail for each mSupply sync run."""

    __tablename__ = "msupply_sync_log"
    ...
    status: Mapped[MSupplySyncStatus] = mapped_column(
        Enum(MSupplySyncStatus, name="msupply_sync_status"),
        nullable=False,
